...
from load_spe3r import load_spe3r_data
...
def config_parser():
    ...
    parser.add_argument("--dataset_type", type=str, default='llff', 
                        help='options: llff / blender / deepvoxels / LINEMOD / spe3r')
    parser.add_argument("--testskip", type=int, default=8, 
                        help='will load 1/N images from test/val sets, useful for large datasets like deepvoxels')
    
    ## spe3r flags
    parser.add_argument("--no_masks", action='store_true', 
                        help='do not use masks to remove background for SPE3R dataset')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        if use_masks:
            print('Using masks to remove background')
        else:
            print('Not using masks - keeping original background')
        i_train, i_val, i_test = i_split

        # SPE3R数据集的near/far设置
        # 由于是航天器，相机距离物体较远，设置合适的near/far
        near = 2.0
        far = 8.0
    ...
    # Generate training plots after training is complete
    ...
    def plot_training_metrics(iterations, losses, psnrs, save_dir):
        ...
        # Plot PSNR
        ax2.plot(iterations, psnrs, 'r-', linewidth=2, label='Training PSNR')
        ...
        # Normalize loss for better visualization alongside PSNR
        if len(losses) > 0 and len(psnrs) > 0:
            ax2 = ax.twinx()
            
            line1 = ax.plot(iterations, losses, 'b-', linewidth=2, label='Loss')
            ...
            ax.set_title('Training Loss and PSNR vs Iteration')
    ...
    def save_training_data(iterations, losses, psnrs, save_dir):
        """Save training data as CSV for future analysis"""
        import csv
        
        csv_path = os.path.join(save_dir, 'training_data.csv')
        with open(csv_path, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['Iteration', 'Loss', 'PSNR'])
